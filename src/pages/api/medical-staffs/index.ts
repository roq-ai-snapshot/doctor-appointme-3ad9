...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    case 'GET':
      return getMedicalStaffs();
    case 'POST':
      return createMedicalStaff();
    default:
  ...
  async function getMedicalStaffs() {
    ...
    const response = await prisma.medical_staff
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'medical_staff'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createMedicalStaff() {
    ...
    const data = await prisma.medical_staff.create({
      data: body,
    });
